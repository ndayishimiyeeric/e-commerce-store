...
export async function PATCH(
  ...
  try {
    ...
    if (!label || !imageUrl) {
      return new Response("Bad Request Both label and image are required", {
        status: 400,
      });
    }

    if (!params.billboardId) {
      return new Response("Bad Request billboard id is required", {
        status: 400,
      });
    }
    ...
    return new Response(
      "Internal server error, can not create billboard at this time",
      { status: 500 }
    );
  }
}
...
export async function GET(
  ...
  try {
    ...
    return new Response(
      "Internal server error, can not get billboard at this time",
      { status: 500 }
    );
  }
}
